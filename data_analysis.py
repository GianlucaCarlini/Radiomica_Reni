...
toolbox.register("mutate", tools.mutFlipBit, indpb=1.0 / (radiomics_venosa.shape[1]))
...
selected_rad = radiomics_venosa.iloc[:, selected]

# %% score

scv = StratifiedKFold(n_splits=10)
score = cross_val_score(
    dtc, X=selected_rad, y=classes_venosa, cv=scv, scoring="roc_auc"
)
...
dtc.fit(selected_rad, classes_venosa)
...
viz.save("decision_tree_venosa.svg")

# %% Save

selected_rad["ID"] = ID_venosa.to_list()
selected_rad["Class"] = classes_venosa.to_list()
selected_rad.to_csv("selected_radiomic_venosa.csv", index=False)
